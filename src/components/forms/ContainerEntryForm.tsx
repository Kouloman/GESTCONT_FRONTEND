...
import { mockShippingLineService, mockIsoCodeService, mockContainerService } from '../../services/mockServices';
import LoadingSpinner from '../ui/LoadingSpinner';
import { Check } from 'lucide-react';

// Form validation schema
const ContainerEntrySchema = Yup.object().shape({
  entryDate: Yup.date()
    .max(new Date(), "La date ne peut pas être dans le futur")
    .required("La date d'entrée est requise"),
  shippingLineId: Yup.string()
    .required("L'armateur est requis"),
...
const ContainerEntryForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [shippingLinesResponse, isoCodesResponse] = await Promise.all([
        ...
        if (import.meta.env.DEV) {
          try {
            const [mockShippingLines, mockIsoCodes] = await Promise.all([
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      // Get shipping line name for display purposes
      const shippingLine = shippingLines.find(sl => sl.id === values.shippingLineId);
      const isoCode = isoCodes.find(iso => iso.id === values.isoCodeId);
      
      if (shippingLine) {
        formattedValues.shippingLineName = shippingLine.name;
      }
      ...
      // Try API first
      try {
        await api.post('/containers', formattedValues);
      } catch (error) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6 animate-fadeIn">
      <h2 className="text-xl font-semibold text-gray-800 mb-6">Enregistrement d'entrée de conteneur</h2>
      ...
      <Formik
        ...
        validationSchema={ContainerEntrySchema}
          ...
          <Form className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <ErrorMessage name="shippingLineId" component="div" className="text-red-500 text-xs mt-1" />
              </div>
